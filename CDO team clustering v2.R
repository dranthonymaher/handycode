...
h1=hist(numericdata[,i], main = colnames(numericdata)[i], xlab =colnames(numericdata)[i]
     ...
     xlim = c(0*min(numericdata[,i])*1.2,max(numericdata[,i])*1.2))
for (k in 1:length(h1$counts)){
  if (h1$counts[k]==0){
    myseq<-0} else {
      if (h1$counts[k]==1){
        myseq<-0} else {
          myseq<-seq(0,(h1$counts[k]-1))
        }
    }
  
  q1<-c(q1,myseq)
}
text(numericdata[,i],q1,df[,7], srt=0, pos=4, cex = 1.5)
dev.off()
}
